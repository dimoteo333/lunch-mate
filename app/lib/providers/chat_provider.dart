...
class ChatState {
  ...
  final List<UserBriefModel>? members;
...
final chatProvider = StateNotifierProvider.family
    .autoDispose<ChatNotifier, ChatState, String>((ref, roomId) {
      return ChatNotifier(ref.read(chatRepositoryProvider), ref, roomId);
    });
...
class ChatNotifier extends StateNotifier<ChatState> {
  ...
  Future<void> loadMembers() async {
    try {
      final members = await _repository.getRoomMembers(roomId);
      state = state.copyWith(members: members);
    } catch (e) {
      debugPrint('Failed to load chat members: $e');
    }
  }
  ...
  Future<void> connectWebSocket() async {
    ...
    final wsUrl =
        '$wsScheme://${uri.host}:${uri.port}/ws/chat/$roomId?token=$token';
